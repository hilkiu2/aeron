...
// end::client[]
{
     ...
     */
    public BasicAuctionClusterClient(final long itemId, final int numOfBids, final int bidIntervalMs)
    {
        this.itemId = itemId;
        ...
        final Header header)
    {
        ...
        printOutput(
            ...
            itemId + ", " + currentPrice + ", " + bidSucceed + ")");
    }
    ...
    private void bidInAuction(final AeronCluster aeronCluster)
    {
        ...
        while (!Thread.currentThread().isInterrupted())
        {
            ...
            if (nextBidDeadlineMs <= currentTimeMs && bidsLeftToSend > 0)
            {
                ...
                printOutput(
                    "Sent(" + (correlationId) + ", " + itemId + ", " + price + ") bidsRemaining=" +
                    bidsLeftToSend);
            }
    ...
    private long sendBid(final AeronCluster aeronCluster, final long price)
    {
        ...
        actionBidBuffer.putLong(ITEM_ID_OFFSET, itemId);
    ...
    {
        final int itemId = Integer.parseInt(System.getProperty("aeron.cluster.tutorial.itemId"));       // <1>
        ...
        final BasicAuctionClusterClient client = new BasicAuctionClusterClient(itemId, numOfBids, bidIntervalMs);
